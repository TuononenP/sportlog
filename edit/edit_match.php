...
print ('<body onload="document.edit_match.match.focus()">');
include "./middle.php";
if (isset($_POST['submit_edit'])) {
	$_SESSION['match_edit'] = $mysqli->real_escape_string($_POST['match']);
...
if (isset($_POST['submit']) && $_POST['submit'] == "Submit") {
	...
	&& $_POST['home_team']!=$_POST['guest_team']) {
		...
		function IsSameMatch() {
			global $match, $date;
			$comp1 = strcmp($match, $_SESSION['match_edit']);
...
<form name="edit_match" id="edit_match"
...
<?php echo $row['Team']; ?>
		...
		<div class="req_fields_text">
			* Required fields
			...
			$query = "CALL EditMatch('" .$_SESSION['match_edit']. "',
'" .$_SESSION['date_edit']. "', '$match', '$date',
$time, '$sport', '$city', '$country', '$place', $description)";
if($mysqli->multi_query($query)) {
	do {
		$result = $mysqli->store_result();
		if($result) {
...
$query = "CALL EditHomeTeam('" .$_SESSION['home_team_old']. "',
'" .$_SESSION['match_edit']. "', '" .$_SESSION['date_edit']. "',
'$home_team', '$match', '$date') ";
if($mysqli->multi_query($query)) {
	do {
		$result = $mysqli->store_result();
		if($result) {
...
$query = "CALL EditGuestTeam('" .$_SESSION['guest_team_old']. "',
'" .$_SESSION['match_edit']. "', '" .$_SESSION['date_edit']. "',
'$guest_team', '$match', '$date') ";
if($mysqli->multi_query($query)) {
	do {
		$result = $mysqli->store_result();
		if($result) {
...
<?php echo $row['Team']; ?>
	...
	$query = "CALL GetMatch('" .$_SESSION['match_edit']. "',
'" .$_SESSION['date_edit']. "')";
	...
	$query = "CALL GetHomeTeam('" .$_SESSION['match_edit']. "',
'" .$_SESSION['date_edit']. "')";
	//get old values and save to variables
	if($mysqli->multi_query($query)) {
		do {
			...
			if($result) {
				$row = $result->fetch_array();
				$home_team_old = $row['Team'];
				$_SESSION['home_team_old'] = $home_team_old;
				$result->close();
			}
		} while($mysqli->next_result());
	}
	$query = "CALL GetGuestTeam('" .$_SESSION['match_edit']. "',
'" .$_SESSION['date_edit']. "')";
	//get old values and save to variables
	if($mysqli->multi_query($query)) {
		do {
			...
			if($result) {
				$row = $result->fetch_array();
				$guest_team_old = $row['Team'];
				$_SESSION['guest_team_old'] = $guest_team_old;
				$result->close();
			}
		} while($mysqli->next_result());
	}
...
<form name="edit_match" id="edit_match"
	...
	<fieldset>
		<label for="match"> Match: <input type="text" name="match" id="match"
			value="<?php echo $match_old; ?>" style="width: 250px" /> * </label>
		<label for="date"> Date: <select name="month" id="month">
			...
			name="home_team" id="home_team">
				<option value="<?php echo $home_team_old; ?>">
				<?php echo $home_team_old; ?>
				</option>
				<option value="">
					Select
				</option>
			...
			name="guest_team" id="guest_team">
				<option value="<?php echo $guest_team_old; ?>">
				<?php echo $guest_team_old; ?>
				</option>
				<option value="">
					Select
				</option>
			...
			name="city" id="city" value="<?php echo $city_old; ?>" /> * </label>
		<label for="country"> Country: <input type="text" name="country"
			id="country" value="<?php echo $country_old; ?>" /> * </label> <label
			for="place"> Place: <input type="text" name="place" id="place"
			value="<?php echo $place_old; ?>" style="width: 250px" /> * </label>
		<label for="description"> Description: <br /> <textarea
				name="description" id="description" rows="5" cols="40">
